...
internal actual object DownloadsPlatformDownloader {
    ...
    // A completed transfer's localFileUri already points wherever AndroidDownloadScheduler put it
    // (internal storage, or — once it relocates a finished file — the user's chosen SAF folder), so
    ...
    actual fun resolveLocalFileUri(localFileUri: String?, destinationFileName: String): String? {
        ...
        val customLocationUri = DownloadsSettingsRepository.downloadLocationUri.value
            ...
            ?: return null
        return DocumentFile.fromTreeUri(context, customLocationUri)
            ?.findFile(fileName)
            ?.takeIf { it.exists() }
            ?.uri
            ?.toString()
